...
def write_yaml_schedule(table: list[list], name: str, start_date: date, end_date: date|None = None, schedule_path: Path = Path("schedule.yaml")):
    ...
    for i, row in enumerate(table[1:], 1):
        ...
        try:
            ...
            for day_idx, day_abbrev in enumerate(day_map):
                if day_idx + 3 < len(row):
                    try:
                        ...
                        print(f"WARNING: Could not parse service availability in row {i}, column {day_idx + 3}: '{row[day_idx + 3]}' - {service_error}")
            ...
            print(f"Added ferry: {leave_chebeague} on {service_days}")
            ...
            print(f"ERROR: Skipping row {i} (time: '{row[1] if len(row) > 1 else 'N/A'}'): {e}")
            continue
    
    # Remove existing schedule with same start date
    data['services']['ctc']['schedules'] = [s for s in data['services']['ctc']['schedules'] if s.get('start') != start_date]
